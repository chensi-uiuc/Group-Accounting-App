...
def _print_welcome_caption(console):
    console.print("Welcome to Group Accounting version 0.6", style="green")
...
def _print_tutorial(console):
    ...
    console.print(f"7. Enter \"{utils.CALC}\" to start calculating and show the results;", style="white bold")
    console.print(f"8. Enter \"{utils.CALC_CLEAN}\" to start calculating (hiding internal process) and show the results;", style="white bold")
    console.print(f"9. Enter \"{utils.SHOW_PEOPLE}\" to check current people;", style="white bold")
    console.print(f"10. Enter \"{utils.SHOW_BILLS}\" to check current bills;", style="white bold")
    # _print_h_line(console, "white")
...
if __name__ == '__main__':
    ...
    while (run_flag == True):
        ...
        if (input_txt == utils.TUT_CHAR):
            ...
            _print_h_line(console, "white")
            console.print("Who Pay:", style="bold")
            payer=[]
            while True:
                name = _get_input(code=1)
                if len(name)==0:
            ...
            _print_h_line(console, "white")
            console.print("Who Enjoy:", style="bold")
            spender=[]
            while True:
                name = _get_input(code=1)
                if len(name)==0:
        ...
        elif (input_txt == utils.SHOW_PEOPLE):
            if acct_system==None:
                console.print("ERROR: No Accounting systems found, please start a new accounting system!", style="bold red")
                continue
            
            table = Table(title=f"Current People")
        ...
        elif (input_txt == utils.SHOW_BILLS):
            if acct_system==None:
                console.print("ERROR: No Accounting systems found, please start a new accounting system!", style="bold red")
                continue

            table = Table(title=f"Current Bill")
            table.add_column("Who Pay")
            table.add_column("Who Enjoy")
            table.add_column("Issue")
            table.add_column("Amount")
            bills = acct_system.get_bills()
            for b in bills:
                table.add_row(','.join(b.get_payer()), ','.join(b.get_spender()), b.get_issue(), str(b.get_amount()))
            console.print(table)
            
        else:
            console.print("ERROR: Invalid Command. Please check the tutorial \"?\"", style="bold red")
