...
def _print_welcome_caption(console):
    console.print("欢迎使用小组活动算账软件 0.6", style="green")
...
def _print_tutorial(console):
    ...
    console.print(f"7. 输入 \"{utils.CALC}\" 开始计算账目（默认开启细节展示）；", style="white bold")
    console.print(f"8. 输入 \"{utils.CALC_CLEAN}\" 开始计算账目（隐藏细节展示）；", style="white bold")
    console.print(f"9. 输入 \"{utils.SHOW_PEOPLE}\" 查看当前人物；", style="white bold")
    console.print(f"10. 输入 \"{utils.SHOW_BILLS}\" 查看当前账目；", style="white bold")
    # _print_h_line(console, "white")
...
if __name__ == '__main__':
    ...
    while (run_flag == True):
        ...
        if (input_txt == utils.TUT_CHAR):
            ...
            table = Table(title=f"当前账目")
            table.add_column("付款者")
            table.add_column("享受服务者")
            table.add_column("事由")
            table.add_column("金额")
            bills = acct_system.get_bills()
            for b in bills:
